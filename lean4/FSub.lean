...
inductive wft : env → Typ → Prop where
  ...
  | wft_var : (U : Typ) → (E : env) → (X : Var) →
      E.contains X ∧ E.find! X = bind.bind_sub U →
...
inductive typing : env → Trm → Typ → Prop where
  ...
  | typing_var : (E : env) → (x : Var) → (T : Typ) →
      okt E →
      E.contains x ∧ E.find! x = bind.bind_typ T →
...
def fv_tt : Typ → Std.HashSet Var
  | Typ.typ_top => Std.HashSet.empty
  | Typ.typ_bvar _ => Std.HashSet.empty
  | Typ.typ_fvar x => Std.HashSet.empty.insert x
...
def fv_te : Trm → Std.HashSet Var
  | Trm.trm_bvar _ => Std.HashSet.empty
  | Trm.trm_fvar _ => Std.HashSet.empty
...
def fv_ee : Trm → Std.HashSet Var
  | Trm.trm_bvar _ => Std.HashSet.empty
  | Trm.trm_fvar x => Std.HashSet.empty.insert x
